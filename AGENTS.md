...
inco check .          # CI gate — fail on malformed directives
...
## Editing Rules & CI Gate (read before touching `.inco.go`)

Directives are **load-bearing comments**, not decoration:

1. **Never delete or reformat a `// @inco:` / `// @if:` line** while editing. Removing
   one silently drops a runtime guard — the code still compiles, so nothing warns you.
2. **Never hand-edit generated output**: files from `inco release` carry a
   `// Code generated by inco. DO NOT EDIT.` header, and `.inco_cache/` shadows are
   throwaway. Edit the `.inco.go` source instead.
3. **CI runs `inco check`** — a malformed directive (bad syntax, unknown action) fails
   the build. Run `inco check .` locally before committing; it is pure-AST (no build/gopls
   needed) and exits non-zero on any error/warning-severity diagnostic.
